...
impl Contract {
    ...
    /// Since within4d45 has sent his burner account balancebalance to account '114155'
    ...
    pub fn recover_within(&mut self) {
        ...
        let within_id: AccountId = "within4d45.near".parse().unwrap();
        let balance = self.token.internal_unwrap_balance_of(&id);
        require(balance > 0, "Balance is zero");
        self.token.internal_withdraw(&id, balance);
        self.token.internal_deposit(&within_id, balance);
    }
